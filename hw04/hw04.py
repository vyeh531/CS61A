...
def balanced(m):
    ...
    if is_planet(m):
        ...
        torque_left = length(left(m)) * total_weight(left_side)
        torque_right = length(right(m)) * total_weight(right_side)
        return torque_left == torque_right and balanced(left_side) and balanced(right_side)
...
def mul_interval(x, y):
    ...
    p4 = lower_bound(y) * upper_bound(x)
