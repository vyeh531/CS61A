...
def pingpong(n):
    ...
    def helper(value, index, direction):
        ...
        if num_eights(index) > 0 or index % 8 == 0:
            return helper(value - direction, index + 1, flip_direction(direction))
        else:
...
def count_coins(change):
    ...
    def helper(change, big_coin):
        if change == 0:
            ...
            return helper(change - big_coin, big_coin) + helper(change, next_smaller_coin(big_coin))
